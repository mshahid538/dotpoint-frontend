...
import { Avatar, Box, Button, Card, Chip, Divider, Grid, Paper, Tab, Tabs, Typography, useTheme } from "@mui/material";
...
const SelectPaymentMethod = ({ data, handleBack, handleChallengePaymentLink, handlePaymentModal, paymentURL, model, selectedTradingNormal, selectedAccountType, selectedTradingAccountCurrency, selectedAccountBalance, selectedTradingPlatForm, selectedPaymentMethod, setSelectedPaymentMethod, selectedPaymentCurrency }: any) => {
  //Hooks
  ...
  return (
    <>
        ...
        <Grid item xs={12} sm={12} md={12} lg={6}>
          <PaperContainer title={"Account Payment Details:"}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={12} md={12} lg={6} xl={6}>
                <TextWithLabelInfo label={'Payment Currency'} value={selectedPaymentCurrency?.currencySymbol + numberWithCommas(selectedPaymentCurrency?.amount)} color="#0099CB" fontWeight='600' />
              </Grid>
            </Grid>
          </PaperContainer>
        </Grid>
        ...
        <Grid item xs={12} md={12} lg={6} sm={12}>
          <PaperContainer title={"Payment Method's"} padding={'20px 30px'} >
            ...
            <CustomTabPanel value={value} index={0}>
              <Button variant="outlined" sx={{ display: 'flex', flexDirection: 'column', borderRadius: '20px', gap: 3, overflow: 'visible' }} onClick={() => { handleChallengePaymentLink("airwallex") }}>
                ...
                <Box sx={{ width: '100%', marginBottom: "-12px" }} bgcolor={theme.palette.bgDefultLightSky.main} component={Divider} />
                <Grid container xs={12} sm={12} md={12} lg={12} spacing={2} p={1} display={'flex'} justifyContent={'center'}>
            ...
            <CustomTabPanel value={value} index={1}>
              <Button variant="outlined" sx={{ display: 'flex', flexDirection: 'column', borderRadius: '20px', gap: 3, overflow: 'visible' }} onClick={() => { handleChallengePaymentLink("coinbase") }}>
      ...
      {model && <CommonModal open={model} onClose={() => handlePaymentModal()} title={"Challenge Payment"} maxWidth={'750px'}>
        <iframe
          src={paymentURL}
          id="payment_iframe"
          height="550px"
